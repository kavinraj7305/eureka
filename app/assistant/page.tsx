...
export default function AssistantPage() {
	...
	const [ideasJson, setIdeasJson] = useState<IdeaCandidate[] | null>(null);
	...
	const submitStep1 = async () => {
		...
		try {
			...
			let candidates: IdeaCandidate[] | null = null;
			try {
				const jsonMatch = (data.reply || "").match(/```json\s*([\s\S]*?)```/i);
				if (jsonMatch) {
					const parsed = JSON.parse(jsonMatch[1]) as unknown;
					if (Array.isArray(parsed)) candidates = parsed as IdeaCandidate[];
				}
			} catch {}
	...
	const downloadPpt = async () => {
		...
		try {
			...
			const message = e instanceof Error ? e.message : "Failed to generate PPT";
			alert(message);
		}
	};
					...
					<div className="mt-8 rounded-xl border border-black/10 dark:border-white/15 p-5 bg-white/70 dark:bg-zinc-900/70 backdrop-blur space-y-4">
						...
						<FocusedInput
							onSend={async (text) => {
								...
								const newMsgs: { role: "user" | "assistant"; content: string }[] = [...focusedMessages, { role: "user", content: text.trim() }];
								...
								try {
									...
									const message = e instanceof Error ? e.message : "Unexpected";
									setFocusedMessages((prev) => [...prev, { role: "assistant", content: `Error: ${message}` }]);
								}
							}}
						/>
						<div className="pt-2">
						...
						<div className="pt-2 flex gap-2">
							...
							<button
								onClick={async () => {
									...
									let structured: IdeaCandidate | null = null;
									...
									try {
										...
										const data0: { reply?: string; error?: string } = await res0.json();
										...
										structured = JSON.parse(jsonText) as IdeaCandidate;
									} catch {
										...
										structured = { title: fallbackTitle, validation: [], ratings: {}, notes } as IdeaCandidate;
									}
									...
									if (!res.ok) {
										const data = await res.json().catch(() => ({} as { error?: string }));
										alert((data as { error?: string })?.error || "Failed to generate PPT");
...
function extractIdeaTitles(md: string): string[] {
	...
	const i = lines.findIndex((l) => /^\s*###\s*Tailored idea directions/i.test(l));
	if (i === -1) return [];
	const titles: string[] = [];
